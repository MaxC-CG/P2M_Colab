...
def plot_pointcloud(filepath):
    # 读取ply, 点云可视化
    vs, normal=read_pts_from_pc(filepath)
    v2=vs
...
apath = os.path.abspath(os.path.join(os.getcwd(),".."))
# pc_path=apath+"/data/g.ply"
# vs, normal=read_pts(pc_path)
# # add_noise_to_pc(vs)
# plot_pointcloud_from_array(vs)

input_path = apath+"/data/bull_initmesh.obj"
output_path = apath+"/data/bull_initmesh_pc.ply"
output_noise_path = apath+"/data/bull_noise.ply"

device = torch.device('cpu')
mesh = load_objs_as_meshes([input_path], device=device)
num_samples=20000 # 采样数
